 ...
 */
export function findLocalStringRanges(fileContent: string, searchString: string, uri: string, commentMatches?: Match[], wholeWord: boolean = false): FileRange[] {
    ...
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        let varIndex = line.indexOf(searchString);
        while (varIndex !== -1) {
            const varEnd = varIndex + searchString.length;
            ...
            varIndex = line.indexOf(searchString, varEnd);
        }
    }
    return ranges;
}
